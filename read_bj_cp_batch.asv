            ...
            for epoch=4:4
                ...
                for subPeriod=1:length(periods)-1
                    ...
                    for i=1:length(channels)
                        ...
                        if animalInd==1&&cond==length(closeCond)&&i==1
                            for cond=1:length(closeCond)
                                yLimVals=get(gca,'ylim');
                                xLimVals=get(gca,'xlim');
                                unitSpace=(yLimVals(2)-yLimVals(1))/20;
                                text('Position',[xLimVals(2)+(xLimVals(2)-xLimVals(1))/25 yLimVals(2)-unitSpace*(length(closeCond)-cond)*2],'FontSize',9,'String',[markerText,'  ',num2str(testContrast(closeCond(cond))),'%'],'Color',colmapText(closeCond(cond),:));
                            end
                        end
...
if roving==0
    ...
    for animalInd=1:2
        sTableRow=[];
        for cond=1:6
            sTableRow=[sTableRow meanCP{animalInd,1}(cond) pPointfive{animalInd,1}(cond)];
        end
    ...
    for animalInd=1:2
        sTableRow=[];
        for cond=1:4
            sTableRow=[sTableRow meanCP{animalInd,2}(cond) pPointfive{animalInd,2}(cond)];
        end
