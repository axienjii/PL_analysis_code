    ...
    for areaInd=1:2
        ...
        for epoch=1:size(test_epochs,2)
            if strcmp(analysisType,'CRF')||strcmp(analysisType,'ROC')&&epoch==4
                ...
                saveMatName=['all_',animal,'_',area,'_',analysisType,'_coefs'];
                subFolder=[analysisType,'_coef_mat'];
                saveMatFolder=fullfile('F:','PL',analysisType,animal,subFolder);
                ...
                for k=1:size(coefficients,2)
                    figText=sprintf('figure(%s)',figureHandles{k});
                    eval(figText)
                    saveCoefImageName=['all_',animal,'_',area,'_',analysisType,'_coefs',startEndTime,saveImageAppendText{k}];
