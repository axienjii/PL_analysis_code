function plot_BU_PL_diff2(PL,BU)
%Written by Xing on 17/08/13, modified from plot_BU_PL_diff, which previously 
%calculated PROBMAT activity incorrectly. Current function combines activity 
%across channels for each trial. Plots ROC curves for PL stimuli, during
...
%                                         eval(saveText);
                            ...
                            for trialInd=1:size(matarray{condInd,2},1)
                                test_act(i,trialInd)=length(matarray{condInd,4}{trialInd});
                                sample_act(i,trialInd)=length(matarray{condInd,2}{trialInd});
                    ...
                    %after activity levels have been read from all
                    %channels, combine activity across channels for each
                    %trial:
                    sum_test_act=sum(test_act,1);%sum across channels (across rows), as each column represents a trial
                    sum_sample_act=sum(sample_act,1);                    
                    roc=sum(sum_test_act>sum_sample_act)/(sum(sum_test_act>sum_sample_act)+sum(sum_test_act<sum_sample_act));%new trial-wise method
                    rocvals=[rocvals roc];
                end
...
%     stdROCb=std(allChSessROCb,1)%bottom-up attention task (attend-away, orientation)
    ...
    elseif samePlot==1
        if areaCount==1
            allMeanROCFig=figure('Color',[1,1,1],'Units','Normalized','Position',[0.12, 0.08, 0.5, 0.35]);
        else
...
if samePlot==1
    ...
    ylim([0 1])
    axis square
    plot(43,0.25,'bo','MarkerFaceColor','b');
    plot(43,0.18,'ro','MarkerFaceColor','r');
    text(45,0.25,'attend-RF');
    text(45,0.18,'attend-away');
