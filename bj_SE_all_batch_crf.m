function bj_SE_all_batch_crf(animal,area)
...
for h=1:length(channels)
    ...
    for sampleContrastsInd=1:length(sampleContrasts)
        ...
        for i=1:length(sessionNums)
            ...
            chStr=[num2str(channel),'_',num2str(sessionNums(i)),'_',num2str(sampleContrast)];
            listing=dir(matFolder);
            matExists=0;
...
%                         cellEpochTimes=epochTimes(row,3);
                ...
                eval(valsText);
                bj_SE_crf(channels(h),sessionNums(i),cellEpochTimes,minusSpon,matarray,animal,area,sampleContrast)
            end
        end
    end
end
