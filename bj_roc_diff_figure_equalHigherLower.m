function bj_roc_diff_figure_equalHigherLower(exampleFig,cutoff,animals,useISI,areas,excludeSuppressed,normalize)
%Written by Xing 01/10/13
%Tests effects of allocating trials where test and sample responses were
%equal in size to one category or the other.
%criterionType=0: original way, exclude 'response equal' trials.
%criterionType=1: add these trials to 'test lower' category.
%criterionType=2: add these trials to 'test higher' category.
            ...
            for sampleContrastsInd=1:length(sampleContrasts)
                ...
                for criterionType=0:2
                    ...
                    for condInd=1:length(testContrast)
                        ...
                        elseif criterionType==1
                            plot(1:size(rocvals,1),rocvals(:,condInd),'bo','MarkerFaceColor','b');hold on
                        elseif criterionType==2
                            plot(1:size(rocvals,1),rocvals(:,condInd),'bo','MarkerFaceColor','k');hold on
                        end
    ...
    for areaInd=1:length(areas)
        ...
        for sampleContrastsInd=1:length(sampleContrasts)
            ...
            for condInd=1:length(testContrast)
                ...
                plot(testContrast(condInd),allROCvals{areaInd,animalInd}(:,condInd),'Color',colmapText(condInd,:),'Marker','.');hold on
                plot(testContrast(condInd),allROCvals{areaInd,animalInd}(:,condInd),'b.');hold on
                ...
                plot(testContrast(condInd),rocvals_sglroc3_allCumulative(condInd),'ro');
                plot(testContrast(condInd),rocvals_allCumulative(condInd),'bo');
            end
