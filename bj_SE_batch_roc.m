function bj_SE_batch_roc(animal,area,channels,sessions,icopy,ncopies,istart)
...
%     try
        ...
        if ~exist(output_fname, 'file')
            bj_SE_V1_2_roc4(animal, channel, session, max(0,verbose-1))% if not, create it
        elseif rewriteSessions==1 
            bj_SE_V1_2_roc4(animal, channel, session, max(0,verbose-1))% or overwrite it
        end        
%     catch ME
