function plot_bj_nvp_PROBMAT_batch_across_chs(roving,areas,psychoType)

%written 25/07/13, modified from plot_bj_nvp_batch_across_chs.
%Reads neuro- and psychometric threshold values for all conditions, based
%on PROBMAT values for population-wide data (previous function took average
%across channels).
            ...
            for epoch=1:size(test_epochs,2)
                if strcmp(analysisType,'NVP')&&epoch==4
                    ...
                    for subPeriod=1:length(periods)-1
                        ...
                        Ncells=0;
                        thresholdsPL=threshold82lower(sessPsyInd);
                        thresholdsPH=threshold82higher(sessPsyInd);
                        ...
                        allisNanList=[{isnanListL} {isnanListH}];
                        ...
                        for subplotInd=1:length(subplotRemap)
                            ...
                            if ~isempty(allData{1,subplotInd})
                                ...
                                if subplotInd<=2
                                    if plotPsychoOnly==0
                                        plot(allXData{1,subplotInd}+startPoint,allData{1,subplotInd},'Marker','o','Color',markerCols(subplotInd),'LineStyle','none','MarkerFaceColor','none');hold on
                                    end
                                else
                                    if plotNeuroOnly==0
                                        for sessionNum=1:length(allData{1,subplotInd})
                                            if allData{1,subplotInd}(sessionNum)==overThreshold(subplotInd)
                                                plot(allXData{1,subplotInd}(sessionNum)+startPoint,allData{1,subplotInd}(sessionNum),'Marker','o','Color',markerCols(subplotInd),'LineStyle','none','MarkerFaceColor','none');hold on
                                            else
                                                plot(allXData{1,subplotInd}(sessionNum)+startPoint,allData{1,subplotInd}(sessionNum),'Marker','o','Color',markerCols(subplotInd),'LineStyle','none','MarkerFaceColor',markerCols(subplotInd));hold on
                                            end
                                        end
                                    end
                                end
