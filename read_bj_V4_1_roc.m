function []=read_bj_V4_1_roc(rocname,psychoname,testContrast,sampleContrast,skipSessions,chNum,area,folder,appendText,animal)
...
if excludeSessHighSSE==1
    ...
    if excludeOutliers==1   %within reduced pool of sessions with good SSE, further examine slope and C50 values for outliers
        ...
        ind=sloutliers+c50outliers;%find sessions where slope and/or C50 values are outliers (union)
...
%     subplot(3,numsessions,i);%V1 data
    ...
    X=fminsearch(@fit_weibull,X0,[],testContrast,rocvals,[],'least_square',[1 1 0 0],[20 100 0 0],[1 1 0 0],[-20 10 0 0]);
    slopeNeuro(1,i)=X(1); 
    c50(1,i)=X(2).*(-log((0.5-X(4))/X(3))).^(1/X(1));
...
if length(coefficients1)>1
    ...
    if writeCoefs==1
        ...
        saveText=['save ',rocCoefMatPathname,'.mat coefficients sessionSorted1 slopeNeuro sessionSorted2 slopePsycho matchPsycho'];
        eval(saveText)
    end
end
