function plot_nonroving_bj_behav_figs(roving,areas)
    ...
    for areaInd=1:length(areas)
        ...
        for sampleInd=1:length(sampleContrasts)
            ...
            for i=1:numsessions
                ...
                line(allMeanPerf(i,17),0:0.01:1,'Color','r');
                ...
                if strcmp(animal,'jack')&&strcmp(area,'v4_1')
                    if i==1||i==2
                        ...
                        %         plot([allMeanPerf(i,17) allMeanPerf(i,17)],[0 1],'Color','r','LineStyle','--');
                        ylim([min(prop_corr),max(prop_corr)]);
                        %     subplottitle=num2str(allMeanPerf(i,1));
                        subplottitle=num2str(i);
                        title(subplottitle);
            ...
            if strcmp(area,'v4_2')
                ...
                [hperfIndpair(plotAreaInd+2*(animalInd-1),1),pperfIndpair(plotAreaInd+2*(animalInd-1),1)]=ttest(allMeanPerf(:,27+numconds*5),allMeanPerf(:,34+numconds*6));
            end
            ...
            if strcmp(area,'v4_2')
                ...
                [hperfInd(plotAreaInd+2*(animalInd-1),1),pperfInd(plotAreaInd+2*(animalInd-1),1)]=ttest(pcInd);
            end
            hold on
        ...
        subplot(2,2,animalInd+2*(plotAreaInd-1));
        f = fittype('a+b*x');
        colTexts='ymcrgbk';
        markerTexts='+x';
        markerSizes=[5 6];
        ...
        if strcmp(area,'v4_2')
            ...
            [rmeanRT(plotAreaInd*2,1),pmeanRT(plotAreaInd*2,1)]=corr(sessions,allMeanPerf(:,23+numconds*3)/1000);
        end
        hold on
        ...
        if strcmp(area,'v4_2')
            ...
            sessions=1:size(allMeanPerf,1);sessions=sessions';
            [rmeanRT(plotAreaInd*2-1,1),pmeanRT(plotAreaInd*2-1,1)]=corr(sessions,allMeanPerf(:,7+numconds)/1000);
        end
        hold on
        ...
        [hdiffCERT(areaInd*2-1,animalInd),pdiffCERT(areaInd*2-1,animalInd)]=ttest(lowerdiffCERT);%blanco V4_1
...
%             end
        ...
        if ~strcmp(area,'v4_2')
            ...
            subplot(1,2,plotAreaInd);
            animalMarkerFaceColors={'none' 'k'};
            plot(1:size(allMeanPerf,1),allMeanPerf(:,116)/1000,'ko','MarkerEdgeColor','k','Color','k','MarkerFaceColor',animalMarkerFaceColors{animalInd});hold on
            plot(1:size(allMeanPerf,1),allMeanPerf(:,137)/1000,'ro','MarkerEdgeColor','r','Color','r','MarkerFaceColor',animalMarkerFaceColors{animalInd});hold on
            ...
            subplot(1,2,plotAreaInd);
            plot(1:size(allMeanPerf,1),(allMeanPerf(:,116)-allMeanPerf(:,137))./allMeanPerf(:,137),'ko','MarkerEdgeColor','k','MarkerFaceColor',animalMarkerFaceColors{animalInd});hold on
            line([0 30],[0 0],'LineStyle',':');
            [h,pRTind(animalInd+2*(plotAreaInd-1)),ciRTind(animalInd+2*(plotAreaInd-1),:),stats] = ttest(allMeanPerf(:,116),allMeanPerf(:,137))
        end
...
%         plot(xtest,ytest)
        ...
        if strcmp(area,'v4_2')
            ...
            plot(1:size(allMeanPerf,1),allMeanPerf(:,17),'ok','MarkerFaceColor',animalMarkerFaceColors{animalInd});
            hold on
            if strcmp(area,'V4_2')%horizontal Gabor
                animalMarkerFaceColors={'none' 'b'};
                plot(size(allMeanPerf,1),allMeanPerf(end,17),'Marker','o','Color','b','LineStyle','none','MarkerFaceColor',animalMarkerFaceColors{animalInd});
            end
            [chiselinearpse(animalInd+2*(plotAreaInd-1)) chisepse(animalInd+2*(plotAreaInd-1))]=bj_linearexpo_fitting(testContrast,allMeanPerf(:,17),0,0,'ROC');
        ...
        %slope
        figure(pcpseslFig)
        subplot(2,3,2+(plotAreaInd-1)*3);
        animalMarkerFaceColors={'none' 'k'};
        if strcmp(area,'v4_2')
            plot(3+maxV4_1(animalInd):size(allMeanPerf,1)+maxV4_1(animalInd)+2,allMeanPerf(:,18),'ok','MarkerFaceColor',animalMarkerFaceColors{animalInd});
            if strcmp(animal,'jack')
                xlim([0 38]);
                ylim([1 11]);
            end
        else
            plot(1:size(allMeanPerf,1),allMeanPerf(:,18),'ok','MarkerFaceColor',animalMarkerFaceColors{animalInd});
            hold on
            if strcmp(area,'V4_2')%horizontal Gabor
                animalMarkerFaceColors={'none' 'b'};
                plot(size(allMeanPerf,1),allMeanPerf(end,18),'Marker','o','Color','b','LineStyle','none','MarkerFaceColor',animalMarkerFaceColors{animalInd});
            end
            [chiselinearsl(animalInd+2*(plotAreaInd-1)) chisesl(animalInd+2*(plotAreaInd-1))]=bj_linearexpo_fitting(testContrast,allMeanPerf(:,18),0,0,'ROC');
            if strcmp(area,'v4_1')&&strcmp(animal,'jack')
                xlim([0 31]);
                ylim([2 14]);
            end
        ...
        if strcmp(area,'v4_2')
            ...
            [hperfIndpair(plotAreaInd+2*(animalInd-1),2),pperfIndpair(plotAreaInd+2*(animalInd-1),2)]=ttest(allMeanPerf(:,29+numconds*6),allMeanPerf(:,36+numconds*7));
        end
        subplot(2,3,3+(plotAreaInd-1)*3);
        animalMarkerFaceColors={'none' 'k'};
        if strcmp(area,'v4_2')
        ...
        if strcmp(area,'v4_2')
            ...
            [hperfIndpair(plotAreaInd+2*(animalInd-1),3),pperfIndpair(plotAreaInd+2*(animalInd-1),3)]=ttest(allMeanPerf(:,28+numconds*6),allMeanPerf(:,35+numconds*7));
        end
        ...
        if strcmp(area,'v4_2')
            ...
            [hperfInd(plotAreaInd+2*(animalInd-1),2),pperfInd(1,2)]=ttest(slInd);
        end
        hold on
        subplot(2,3,3+(plotAreaInd-1)*3);
        animalMarkerFaceColors={'none' 'k'};
        ...
        if strcmp(area,'v4_2')
            ...
            [hperfInd(plotAreaInd+2*(animalInd-1),3),pperfInd(1,3)]=ttest(pseInd);
        end
        hold on
    end
end
...
if printFigs==1
    if roving==0
        rovingText='_non_roving';
    elseif roving==1
        rovingText='_roving';
    end
    folderPrint=fullfile('F:','PL','psycho_data','behavioural_figures');
    formats=[{'epsc'} {'png'}];
    for i=1:length(formats)
        ...
        for animalInd=1:length(animals)
            for areaInd=1:length(areas)
                figure(psychoCurves(animalInd+2*(areaInd-1)))
                figName=[folderPrint,'\_',animals{animalInd},'_psycho_',areas{areaInd},rovingText];
                printtext=sprintf('print -d%s %s -r600',format,figName);
                set(gcf, 'PaperPositionMode', 'auto');
                eval(printtext);
            end
