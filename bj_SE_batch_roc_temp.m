function bj_SE_batch_roc_temp(animal,area,channels,sessions,icopy,ncopies,istart,drawFigs)
    ...
    try
        ...
        elseif rewriteSessions==1 
            if checkFileDateMod==0
                bj_SE_V1_2_roc4_temp(animal, channel, session, max(0,verbose-1))% overwrite it regardless of last date of modification
            else
                fileinfo=dir(output_fname);
                modDate=fileinfo.date;
                if ~strcmp(modDate(4:6),'Mar')||str2double(modDate(1:2))<19||str2double(modDate(1:2))==19&&str2double(modDate(13:14))<13&&str2double(modDate(16:17))<04
