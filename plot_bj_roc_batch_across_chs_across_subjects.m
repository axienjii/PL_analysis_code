function plot_bj_roc_batch_across_chs_across_subjects(area,analysisType)
%Categories channels according on value of mean early C50s relative to 30%.
%Combines data across subjects but keeps data separate for areas.
        ...
        if strcmp(analysisType,'CRF')||strcmp(analysisType,'ROC')&&epoch==4
            ...
            for subPeriod=1:length(periods)-1
                ...
                for animalInd=1:2
                    ...
                    channelsAllSubjects=[channelsAllSubjects channels];
                    sessionsAllSubjects=[sessionsAllSubjects channels];
                    for buildAnimalList=1:length(channels)
                        animalList=[animalList {animal}];
                    end
                end
                channels=channelsAllSubjects;
                cellCategory=zeros(length(channels),2)-2;
                cellCategory(:,1)=channels;
                sessions=sessionsAllSubjects;              
                ...
                for i=1:length(channels)
                    ...
                    coefsPathName=fullfile('F:','PL',analysisType,animalList{i},subFolder,coefsFileName);
...
saveCoefImageName=['all_subjects_',area,'_',analysisType,'_coefs_C50_categories',startEndTime,includeSigOnlyText];
