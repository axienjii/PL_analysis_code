function read_bj_SE_xcorr5skewslope(folder,blanco_sessions)
...
for row=1:size(allSlopes,1)
    ...
    plot(allSlopes{row,3},allSlopes{row,2},'kx');hold on
    constants=polyfit(allSlopes{row,3},allSlopes{row,2},1);%fit best straight line to data points (least squares method)
    fitLine=constants(1)*allSlopes{row,3}+constants(2);
    plot(allSlopes{row,3},fitLine,'r:');hold on
    a=[allSlopes{row,3}' allSlopes{row,2}'];%check for change in slope with time- expect a +ve correlation, if any
    ...
    title(subplottitle);
    set(gca,'XLim',[min(allSlopes{row,3}) max(allSlopes{row,3})]);
    set(gca,'XTick',[min(allSlopes{row,3}) max(allSlopes{row,3})]);
    set(gca,'XTickLabel',[min(allSlopes{row,3}) max(allSlopes{row,3})]);
    ...
    text('Position',[min(allSlopes{row,3}) max(allSlopes{row,2})-0.1*(max(allSlopes{row,2})-min(allSlopes{row,2}))],'FontSize',7,'String',ptext1);
    ptext2=[' p=',num2str(p1(1,2))];
    text('Position',[min(allSlopes{row,3}) max(allSlopes{row,2})-0.2*(max(allSlopes{row,2})-min(allSlopes{row,2}))],'FontSize',7,'String',ptext2);
    if row==1
        ptext=sprintf('%s    %s',folder,blanco_sessions);
        orient landscape
