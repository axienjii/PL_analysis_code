        ...
        for sampleInd=1:length(sampleContrasts)
            ...
            if strcmp(area,'v4_1')
                if strcmp(animal,'blanco')&&includeV4_0==1
                    allMeanPerfMain=allMeanPerf;%V4_1 data
                    loadText=['load ',rootFolder,'\PL\psycho_data\',animal,'\allMeanPerf\allMeanPerf_v4_0_1_',num2str(sampleContrast),'.mat allMeanPerf'];
                    eval(loadText)
                    allMeanPerf304=[allMeanPerf(1:10) 0 0 0 0 0 0 allMeanPerf(11:end) zeros(1,36)];%add V4_0_1 data (session 304)
                    loadText=['load ',rootFolder,'\PL\psycho_data\',animal,'\allMeanPerf\allMeanPerf_v4_0_2_',num2str(sampleContrast),'.mat allMeanPerf'];
                    eval(loadText)
                    allMeanPerf305=[allMeanPerf(1:14) 0 0 allMeanPerf(15:end) zeros(1,12)];%add V4_0_2 data (session 305)
                    loadText=['load ',rootFolder,'\PL\psycho_data\',animal,'\allMeanPerf\allMeanPerf_v4_0_3_',num2str(sampleContrast),'.mat allMeanPerf'];
                    eval(loadText)
                    allMeanPerf306=allMeanPerf;%add V4_0_3 data (session 306)
                    if include306==1
                        allMeanPerf=[allMeanPerf304;allMeanPerf305;allMeanPerf306;allMeanPerfMain];%add V4_0 data (session 304 to 306 and onwards)
                    elseif include306==0
                        allMeanPerf=[allMeanPerf304;allMeanPerf305;allMeanPerfMain];%add V4_0 data (session 304 to 306 and onwards)
                    end
                elseif strcmp(animal,'jack')&&includeV4_0==1
                    allMeanPerfMain=allMeanPerf;%V4_1 data
                    loadText=['load ',rootFolder,'\PL\psycho_data\',animal,'\allMeanPerf\allMeanPerf_v4_0_1_',num2str(sampleContrast),'.mat allMeanPerf'];
                    eval(loadText)
                    allMeanPerf22=[allMeanPerf(1:10) 0 0 0 0 0 0 allMeanPerf(11:end) zeros(1,36)];%add V4_0_1 data (session 22)
                    loadText=['load ',rootFolder,'\PL\psycho_data\',animal,'\allMeanPerf\allMeanPerf_v4_0_2_',num2str(sampleContrast),'.mat allMeanPerf'];
                    eval(loadText)
                    allMeanPerf23=[allMeanPerf(1:14) 0 0 allMeanPerf(15:end) zeros(1,12)];%add V4_0_2 data (session 23)
                    allMeanPerf=[allMeanPerf22;allMeanPerf23;allMeanPerfMain];%add V4_0 data (session 22, 23, and onwards)
                end
                maxV4_1(animalInd)=size(allMeanPerf,1);
                horizontalPerf=allMeanPerf(end,:);%session with horizontal stimulus
                ...
                if strcmp(animal,'jack')&&strcmp(area,'v4_1')
                    if i==1||i==2
                        ...
                        if i==1
                            ...
                            slopePsycho(1,i)=100*X(1)*X(3)*exp(-(sampleContrast/X(2))^X(1) )*sampleContrast^(X(1)-1)*(1/X(2))^X(1);%multiply by 100 as the input of proportion correct is a fraction
                        else
                            ...
                            yvals=1-X(4)-X(3).*exp(-(xvals./X(2)).^X(1));
                            PSE=X(2).*(-log((0.5-X(4))/X(3))).^(1/X(1));
                            slopePsycho(1,i)=100*X(1)*X(3)*exp(-(sampleContrast/X(2))^X(1) )*sampleContrast^(X(1)-1)*(1/X(2))^X(1);
                        end
                        plot(testContrast,prop_corr,'ok');
                        hold on
                        plot(xvals,yvals,'k');
...
%                     end
            ...
            if strcmp(area,'v4_2')
                ...
                hold on
                if strcmp(area,'v4_1')&&includeV4_0==1%include sessions V4_0 (Blanco: 304 to 306; Jack: 22, 23)
                    animalMarkerFaceColors={'none' 'r'};
                    if strcmp(animal,'blanco')
                        if include306==0
                            plot(1:2,allMeanPerf(1:2,3+length(testContrast)),'or','MarkerFaceColor',animalMarkerFaceColors{animalInd});
                        elseif include306==1
                            plot(1:3,allMeanPerf(1:3,3+length(testContrast)),'or','MarkerFaceColor',animalMarkerFaceColors{animalInd});
                        end
                    elseif strcmp(animal,'jack')
                        plot(1:2,allMeanPerf(1:2,3+length(testContrast)),'or','MarkerFaceColor',animalMarkerFaceColors{animalInd});
                    end
                end
                if strcmp(area,'v4_1')%horizontal Gabor
                    animalMarkerFaceColors={'none' 'b'};
            ...
            if strcmp(area,'v4_2')
                ...
                if strcmp(area,'v4_1')&&includeV4_0==1%include sessions V4_0 (Blanco: 304 to 306; Jack: 22, 23)
                    ...
                    if strcmp(animal,'blanco')
                        if include306==0
                            plot(1:2,allMeanPerf(1:2,4+length(testContrast)),'or','MarkerFaceColor',animalMarkerFaceColors{animalInd});
                        elseif include306==1
                            plot(1:3,allMeanPerf(1:3,4+length(testContrast)),'or','MarkerFaceColor',animalMarkerFaceColors{animalInd});
                        end
                    elseif strcmp(animal,'jack')
                        plot(1:2,allMeanPerf(1:2,4+length(testContrast)),'or','MarkerFaceColor',animalMarkerFaceColors{animalInd});
                    end
